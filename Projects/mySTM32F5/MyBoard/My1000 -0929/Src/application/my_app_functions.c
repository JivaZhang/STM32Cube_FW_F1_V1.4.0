...
float getReceivePower(void){
	uint8     cirPwrBytes[LEN_CIR_PWR];
	uint8     rxFrameInfo[4];
	uint32_t twoPower17 = 131072;
	uint16_t C, N;
	float    A, corrFac;
	dwt_readfromdevice(RX_FQUAL_ID, CIR_PWR_SUB, LEN_CIR_PWR, cirPwrBytes);
	dwt_readfromdevice(RX_FINFO_ID, 0x00, RX_FINFO_LEN, rxFrameInfo);
