...
static uint64 t64;
static float  rxPowerBase    ;
static int16_t	rxPowerBaseLow ;
static int16_t	rxPowerBaseHigh ;
#endif

#ifdef TAG_BLEUART_OUTPUT
uint8 BLEOutput[82];
uint8 Blank6[6]=  {0x00,0x00,0x00,0x00,0x00,0x00};
uint8 Blank10[10]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
uint8_t SPIBuffer[80];
...
void correctTimestamp(uint8* timestamp,uint8);
void sendresult(void);

extern uint32 inittestapplication(uint8 s1switch);
extern uint8 s1switch;
...
//			set_info.txAntennaDelay = INPUT_ANT_DELAY;
    ...
    if((power == 0x0) || (power == 0xFFFFFFFF)) //if there are no calibrated values... need to use defaults
    {
		...
		power=0x0A0A0A0A;//0x1F1F1F1F;//0xBDBDBDBD;//0x85858585;//  
...
void tag_loop(void){
	if(set_info.Time_next_Poll<portGetTickCnt()){
      ...
      SEQ_range++;
			...
			set_info.Time_next_Ranging=current+(1+set_info.active_device_num)*set_info.delayedReplyTime/1000+10;
			set_info.poll_replied_num=0;
			if(set_info.Time_next_blink-current<100){
			  set_info.Time_next_blink=current+100;
			}
		}
		set_info.Time_next_Poll=set_info.Time_next_Poll+POLL_SLEEP_DELAY;
...
//	if(set_info.Time_next_Ranging<portGetTickCnt() && set_info.active_device_num>0){
	if((set_info.poll_replied_num>0 &&(set_info.poll_replied_num==set_info.active_device_num |set_info.Time_next_Ranging<portGetTickCnt()) )){
	  ...
	  set_info.poll_replied_num=0;
		...
		uint32 current=portGetTickCnt();
    set_info.Time_next_Trinagle=portGetTickCnt()+50;
		set_info.Time_next_Ranging=portGetTickCnt()+100000000;
	}
	...
	if(set_info.final_recieved_num>0 &&(set_info.final_recieved_num==set_info.active_device_num |set_info.Time_next_Trinagle<portGetTickCnt()) ){
		MinsqProccess();
		set_info.Time_next_blink=portGetTickCnt()+20;
		...
		for(int i=0;i<MAX_ANC_NUM;i++){
			if(DeviceList[i]._isactivate && DeviceList[i].SEQ_range==SEQ_range){
				memcpy(&BLEOutput[2+i*10],&DeviceList[i]._shortAddress[0],2);
				memcpy(&BLEOutput[2+i*10+2],&DeviceList[i]._range,4);
				memcpy(&BLEOutput[2+i*10+6],&DeviceList[i]._rss,4);
			}else{
				memcpy(&BLEOutput[2+i*10],Blank10,10);
			}
		}
		memcpy(&BLEOutput[2+60],&set_info.CurrentXYZ[0],4);
		memcpy(&BLEOutput[2+60+6],&set_info.CurrentXYZ[1],4);
		memcpy(&BLEOutput[2+60+12],&set_info.CurrentXYZ[2],4);
    BLEOutput[0]=0xff;
		BLEOutput[1]=0xff;
		BLEOutput[80]=0xfe;
		BLEOutput[81]=0xfe;
		writetoLCD( 82, 1,BLEOutput );
#else
		for(int i=0;i<MAX_ANC_NUM;i++){
			if(DeviceList[i]._isactivate && DeviceList[i].SEQ_range==SEQ_range){
				memcpy(&SPIBuffer[i*10],&DeviceList[i]._shortAddress[0],2);
				memcpy(&SPIBuffer[i*10+2],&DeviceList[i]._rss,4);
				memcpy(&SPIBuffer[i*10+6],&DeviceList[i]._range,4);
			}else{
				memcpy(&SPIBuffer[i*10],Blank10,10);
			}

		}
			memcpy(&SPIBuffer[60],&set_info.CurrentXYZ[0],4);
			memcpy(&SPIBuffer[60+6],&set_info.CurrentXYZ[1],4);
			memcpy(&SPIBuffer[60+12],&set_info.CurrentXYZ[2],4);
			if(SPILoadingFlag==0){
				 SPILoadingFlag=1;
				 sendresult();
			 }
		  HAL_GPIO_WritePin(SPI_FLAG_Port, SPI_FLAG_Pin, GPIO_PIN_SET);
#endif		
#endif
...
//#ifdef DEBUG_ENABLE	
//			  	writetoLCD( 1, 1, "1"); //send some data
//#endif
	}
}
void sendRange(void){
...
#endif
			DelayTime=set_info.delayedReplyTime*(loadcounter+1);
...
//			dwt_readrxtimestamp(&(DeviceList[i].timePollReceivedBytes[0]));
		  correctTimestamp(&(DeviceList[i].timePollReceivedBytes[0]),i);
...
//				dwt_readrxtimestamp(&(DeviceList[i].timeRangeReceivedBytes[0]));
				correctTimestamp(&(DeviceList[i].timeRangeReceivedBytes[0]),i);
...
//			dwt_readrxtimestamp(&(DeviceList[i].timePollAckReceivedBytes[0]));
			correctTimestamp(&(DeviceList[i].timePollAckReceivedBytes[0]),i);
		}
	}
}
...
#ifdef RSS_DISPLAY
		  writetoLCD( 1, 1," " );
		  sprintf((char*)rangeDis,"%-10.3f",DeviceList[i]._rss);
		  writetoLCD( 10, 1,rangeDis );
#endif
#endif
		
			if(a<300 &&a >-5){
				memcpy(&DeviceList[i]._range,msg_recieve+12,4);		
...
void process_event(event_data_t event){
	if(event.type==SendRangeInit){
		...
		if(decode_range_AndSolveDis(event)==1){      //return 1 only when Anchor itself in code and Tag in Anchor's list
			...
			writetoLCD( 1, 1," " );
	  sprintf((char*)rangeDis,"%-6.3f",DeviceList[set_info.last_send_tag_idx]._rss);
	...
	}else if(event.type==ReEnableRecieve){
		instancerxon(0,0);
	}
...
//	DeviceList[Dev_idx]._range= DISTANCE_OF_RADIO*DeviceList[Dev_idx].timeT;
	distance=DISTANCE_OF_RADIO*DeviceList[Dev_idx].timeT;
	
#ifdef CORRECT_RANGE_BY_RANGE
	if(set_info.smartPowerEn)
	{
		//1.31 for channel 2 and 1.51 for channel 5
		if(set_info.configData.chan == 5)
		{
	...
	DeviceList[Dev_idx]._range = distance - dwt_getrangebias(set_info.configData.chan, (float) distance_to_correct, set_info.configData.prf);
		...
		sprintf((char*)rangeDis,"%-6.3f",DeviceList[Dev_idx]._range);
		writetoLCD( 6, 1,rangeDis );
...
void correctTimestamp(uint8* timestamp,uint8 ii) {
	DeviceList[ii]._rss=getReceivePower();
#if CORRECT_TIME_BY_RePWR==1
	uint8 t[5];
  ...
  t64= (uint64) t[0]
	...
	#ifdef USE_MEASURED_RANGE_BIAS
				rxPowerBase     = -(DeviceList[ii]._rss+68.0f)*0.5f;
				rxPowerBaseLow  = (int16_t)rxPowerBase; // TODO check type
				rxPowerBaseHigh = rxPowerBaseLow+1; // TODO check type

				if(rxPowerBaseLow < 0) {
					rxPowerBaseLow  = 0;
					rxPowerBaseHigh = 0;
				} else if(rxPowerBaseHigh > 12) {
					rxPowerBaseLow  = 12;
					rxPowerBaseHigh = 12;
				}
				...
				rangeBiasHigh = (rxPowerBaseHigh < BIAS_SELF_MEASURED_ZERO ? -BIAS_SELF_MEASURED[rxPowerBaseHigh] : BIAS_SELF_MEASURED[rxPowerBaseHigh]);
				rangeBiasHigh <<= 1;
				rangeBiasLow  = (rxPowerBaseLow < BIAS_SELF_MEASURED_ZERO ? -BIAS_SELF_MEASURED[rxPowerBaseLow] : BIAS_SELF_MEASURED[rxPowerBaseLow]);
				rangeBiasLow <<= 1;
				...
				rxPowerBase     = -(DeviceList[ii]._rss+61.0f)*0.5f;
				rxPowerBaseLow  = (int16_t)rxPowerBase; // TODO check type
				rxPowerBaseHigh = rxPowerBaseLow+1; // TODO check type
				if(rxPowerBaseLow < 0) {
					rxPowerBaseLow  = 0;
...
void sendresult(void){
//	    SPIBuffer[78]=SEQ_range;
			memcpy(&aTxBuffer[0],&SPIBuffer[0],79);
			aTxBuffer[78]=SEQ_range;
			if(HAL_SPI_TransmitReceive_IT(&hspi2, (uint8_t*)aTxBuffer, (uint8_t *)aRxBuffer, 79) != HAL_OK)
			{
...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi){
	...
	if(hspi==&hspi2){
    HAL_GPIO_WritePin(SPI_FLAG_Port, SPI_FLAG_Pin, GPIO_PIN_RESET);// set to 0
		SPILoadingFlag=0;
	}
#endif
}
